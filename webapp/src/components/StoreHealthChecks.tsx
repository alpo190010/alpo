...
import type { AccessibilityRule, DimensionCheck } from "@/lib/analysis/types";
...
function CheckRow({
  ...
  // Failing rows with a remediation, code snippet, or rule list become
  // expandable disclosures. Any combination may be present.
  const expandable =
    tone === "fail" &&
    Boolean(item.remediation || item.code || (item.rules && item.rules.length));
...
/* ── Per-rule list (axe long-tail) ──────────────────────────── */
function RuleList({ rules }: { rules: AccessibilityRule[] }) {
  return (
...
function isSafeAxeUrl(url: string): boolean {
  if (!url) return false;
  try {
